...
event_description_map = {
    "WorkflowStartedEvent": "요청을 처리하고 있습니다...",
    "getAccountsByUserName": {
        "start": "사용자명으로 계좌를 조회하고 있습니다...",
        "end": "계좌 정보를 조회했습니다"
    },
    "getAccountDetails": {
        "start": "계좌 상세 정보를 가져오고 있습니다...",
        "end": "계좌 상세 정보를 가져왔습니다"
    },
    "getPaymentMethodDetails": {
        "start": "결제 수단 정보를 가져오고 있습니다...",
        "end": "결제 수단 정보를 가져왔습니다"
    },
    "getTransactionsByRecipientName": {
        "start": "수취인 거래 내역을 검색하고 있습니다...",
        "end": "수취인 거래 내역을 찾았습니다"
    },
    "scan_invoice": {
        "start": "업로드된 이미지에서 데이터를 추출하고 있습니다...",
        "end": "이미지에서 데이터를 추출했습니다"
    },
    "processPayment": {
        "start": "결제를 처리하고 있습니다...",
        "end": "결제가 완료되었습니다"
    ...
    "getCreditCards": {
        "start": "신용카드 정보를 조회하고 있습니다...",
        "end": "신용카드 정보를 조회했습니다"
    },
    "getCardDetails": {
        "start": "신용카드 상세 정보를 가져오고 있습니다...",
        "end": "신용카드 상세 정보를 가져왔습니다"
    },
    "getCardTransactions": {
        "start": "신용카드 거래 내역을 조회하고 있습니다...",
        "end": "신용카드 거래 내역을 조회했습니다"
    }
        ...
        async for event in events:
            ...
            if isinstance(event, AgentRunUpdateEvent):
                ...
                if isinstance(event.data, AgentRunResponseUpdate) \
                    ...
                    # type: ignore
                    function_call_content: FunctionCallContent = event.data.contents[0]

                    if function_call_content.name:
                        call_id = function_call_content.call_id
                        self.tool_name_id_map[call_id] = function_call_content.name
                        descriptive_title = event_description_map[function_call_content.name][
                            "start"] if function_call_content.name in event_description_map else function_call_content.name
                        function_call_task = CustomTask(
                            title=descriptive_title, icon="search")
                        taskUpdate = TaskItem(
                            thread_id=thread_id, id=call_id, task=function_call_task, created_at=datetime.now())
                ...
                if isinstance(event.data, AgentRunResponseUpdate) \
                    ...
                    # type: ignore
                    function_result_content: FunctionResultContent = event.data.contents[0]
                    if function_result_content.call_id:
                        tool_name = self.tool_name_id_map.get(
                            function_result_content.call_id, function_result_content.call_id)
                        descriptive_title = event_description_map[tool_name][
                            "end"] if tool_name in event_description_map else tool_name
                        function_result_task = CustomTask(
                            title=descriptive_title, icon="check-circle-filled")
                        taskResultUpdate = TaskItem(
                            thread_id=thread_id, id=function_result_content.call_id, task=function_result_task, created_at=datetime.now())
                ...
                if isinstance(event.data, AgentRunResponseUpdate) \
                    ...
                    # type: ignore
                    function_approval_content: FunctionApprovalRequestContent = event.data.contents[
                        0]
                    ...
                    approval_request_widget = build_approval_request(
                        tool_name=tool_name, tool_args=parsed_args, call_id=function_approval_content.function_call.call_id, request_id=function_approval_content.id)
                   # Server Managed Widget Item
                    ...
                    # type: ignore
                    yield CustomThreadItemDoneEvent(type="thread.item.done", item=client_widget_item)

            else:
                event_description = event_description_map.get(
                    event.__class__.__name__, event.__class__.__name__)
                progressUpdate = ProgressUpdateEvent(
                    text=event_description, icon="atom")
